...
class DB
{
    ...
    public function count($column){
        // dd("SELECT COUNT($column) FROM ".implode(', ', $this->from));
        return self::instance()->query("SELECT COUNT($column) as $column FROM ".implode(', ', $this->from))->fetch();
    }
    ...
    public function fetch() {
        ...
        $this->query .= ($this->conditions === [] ? '' : ' WHERE ' . implode(in_array('=',$this->conditions) ? '' : ' = ', $this->conditions));
    ...
    {
        $this->query = 'DELETE FROM ' . implode(', ', $this->from) . ($this->conditions === [] ? '' : ' WHERE ' . implode(' = ', $this->conditions));
        $this->query = self::instance()->prepare($this->query)->execute();
        return $this->query;
    }
